-- This file is for "standard" effect types (erosion, standard plants, 3D sprawlers) where many are bulked into similarly grouped logic.
        ...
        case tp of
          ...
          "Lollipop Mold":
            if(random(300) > gEEprops.effects[r].mtrx[q2][c2])then
              ...
              stemColor = color(150,0,0) -- stems forced as shaded color
              if layer = 2 then
                stemColor = color(255,0,0) -- shaded color looks ass in l3 so don't actually use forced shaded
              end if
              member("layer"&string(lr)).image.copyPixels(DRPxl, rct, rect(0,0,1,1), {#color:color(150,0,0), #ink:36})
